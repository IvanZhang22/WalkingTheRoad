# 行小道本地 Agent v3.1.1（项目上下文与自然交互）

v3.1.1 在 v3.1.0 的多模态基础上，补齐同一清小搭 `sessionId` 内的项目上下文、命名项目切换/归档恢复、当前菜单绑定与自然语言优先的交互规则。W1—W4 仍负责实际的研究设计、访谈设计、材料分析和证据质检；系统不会编造材料或自动确认研究结论。
