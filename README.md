...
v3.0.0 是面向云服务器部署与清小搭正式接入的研究旅程版：主智能体负责接待、意图澄清、可纠正路由、上传授权、人工确认、异常守门与下一阶段导航；W1—W4 仍负责实际的研究设计、访谈设计、材料分析和证据质检。会话状态使用清小搭 `sessionId` 在服务端保存；内部节点名、协议模式和提示词不向用户展示。
